...
class Tune:
    ...
    def run_input_checks(self):
        ...
        if not self.basis:
            raise Exception('Basis Error: I can\'t tune a molecule without a basis set...')
        if self.basis['option'] == 'global':
            ...
            for i in self.basis['basis']:
                if i[0] != '#':
                    count += 1
            if count != 1:        
                raise Exception('Basis Error: You must have one non-blank/uncommented ' +
        ...
        if self.ecp and self.ecp['option'] == 'global':
            ...
            for i in self.ecp['ecp']:
                if i[0] != '#':
                    count += 1
            if count != 1:
                raise Exception('ECP Error: You must have one non-blank/uncommented ' +
        ...
        if not self.tune:
            raise Exception('Tune Error: I can\'t tune a molecule without a tune block...')
        ...
        if self.tune['dimension'] == 1 and self.tune['step'] not in ['base', 'coarse', 'fine']:
